...
class TeacherDatesheets extends StatefulWidget {
  const TeacherDatesheets({Key? key}) : super(key: key);

  @override
  State<TeacherDatesheets> createState() => _TeacherDatesheetsState();
}

class _TeacherDatesheetsState extends State<TeacherDatesheets> {
  ...
  void getMyData() async {
    await SharedPreferenceManager.getInstance().getUser().then((model) {
      getDateSheets(model.userDepartment);
    });
    setState(() {});
  }
  ...
  getDateSheets(String departmentId) async {
    Provider.of<AppProvider>(context, listen: false).fetchAllDateSheetsTeachers(departmentId, authToken);
  }
